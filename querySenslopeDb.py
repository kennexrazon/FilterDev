...
#        dflgd = dfa.sort(['id']).reset_index(level=1,drop=True)
        dflgd = dfa.sort_values(by='id').reset_index(level=1,drop=True)
...
def GetSingleLGDPM(site, node, startTS):
    query = "SELECT timestamp, '%s' as 'name' , id, xvalue, yvalue, zvalue" %str(site)
    if len(site) == 5:
        query = query + ", msgid"
...
#    print "query inside GetSinglelgdpm: %s " %query
...
#                print 'no data for node ' + str(node) + ' of ' + site
    ...
    if len(site) == 5:
        lgdpm.columns = ['ts','name','id','x','y','z', 'msgid']
    else:
        lgdpm.columns = ['ts','name','id','x','y','z']
        
    lgdpm = lgdpm[['ts','name', 'id', 'x', 'y', 'z']]
#    print "lgdpm ="
#    print lgdpm
